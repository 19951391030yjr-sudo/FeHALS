...
### Added
- 仿真过程动画：平台代理按 HELIOS++ 平台类型适配（旋翼 / 固定翼沿规划航迹飞行、旋翼自转、机头朝向航段方向；车载平台贴地行驶；TLS 三脚架原地静态、转头示意自转）；点云按 HELIOS++ 出点顺序逐步生成（仅改写 geometry.drawRange，零缓冲重建），支持「逐点连续 / 逐条带（按航段）」两种揭示方式
- 「动画」Tab：播放/暂停/从头播放/停止、倍速（0.5~8×）、循环、仿真完成后自动播放、显示开关（平台/航迹/视角跟随）、平台示意大小倍率、航迹与进度统计（长度/航段/时长/平台高度参数/已生成点数）
